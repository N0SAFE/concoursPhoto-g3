...
export default function CompetitionComing() {
    ...
    useEffect(() => {
        const total = organization.competitions.reduce((count, competition) => {
            if (format(new Date(competition.publicationDate), 'yyyy-MM-dd') >= format(now, 'yyyy-MM-dd') && format(new Date(competition.resultsDate), 'yyyy-MM-dd') >= format(now, 'yyyy-MM-dd')) {
                count++;
            }
            return count;
        }, 0);
