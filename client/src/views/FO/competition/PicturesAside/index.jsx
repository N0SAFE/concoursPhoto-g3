...
export default function PicturesAside({ requestType }) {
    ...
    return (
        <div className={style.asideContainer}>
            <div>
                <h2>Dernières photos {requestType === "last-pictures-posted" ? "soumises" : (requestType === "last-pictures-obtained-votes" ? "ayant obtenu un vote" : "ayant obtenu un prix")}</h2>
                <div>
                    {lastPictures.map((picture) => {
                        return (
                            <img src={import.meta.env.VITE_API_URL + "/" + picture.path} alt={picture.competition_name} />
                        )
                    })}
                </div>
            </div>
