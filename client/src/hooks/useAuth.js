        ...
        fetch(new URL(import.meta.env.VITE_API_URL + '/login_check'), {
            ...
            .then(async data => {
                ...
                return await checkLogged().then(({ isLogged, me }) => {
                    if (!isLogged) {
                        if (autoLogoutOnFail) {
                            return logout(checkLogged).then(() => {
                                // this function is used to avoid fail when the refresh token is not the good one so we remove it to recreate it after
                                return login(
                                    checkLogged,
                                    { identifier, password },
                                    false
                                ).then(function () {
                                    checkLogged().then(function ({
                                        isLogged,
                                        me,
                                    }) {
                                        if (!isLogged) {
