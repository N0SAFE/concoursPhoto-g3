            ...
            <div className={style.competitionBanner}>
                ...
                <div className={style.filter}>
                    <div className={style.viewFilter}>
                        <div>
                            <h1>Concours photo "{entity.competition_name}"</h1>
                        </div>
                        <div>
                            <Chip backgroundColor={"#F5F5F5"} title={<Dropdown title="Thème(s) :" links={
                                entity.theme?.map((theme) => ({title: theme.label}))
                            } iconColor={"#000"}/>} />
                            <Chip backgroundColor={"#F5F5F5"} title={`Pays : ${entity.country_criteria}`} />
                            <Chip backgroundColor={"#F5F5F5"} title={<Dropdown title="Région(s) :" links={
                                entity.region_criteria?.map((region) => ({title: region.nom}))
                            } iconColor={"#000"}/>} />
                            <Chip backgroundColor={"#F5F5F5"} title={<Dropdown title="Catégorie(s) :" links={
                                entity.participant_category?.map((category) => ({title: category.label}))
                            } iconColor={"#000"}/>} />
                            <Chip backgroundColor={"#F5F5F5"} title={`Âge : de ${entity.min_age_criteria} à ${entity.max_age_criteria} ans`} />
                            <Chip backgroundColor={"#F5F5F5"} title={`Dotation : ${entity.endowments}`} />
                        </div>
                    </div>
                    <div className={style.viewOrganizer}>
                        <div>
                            ...
                            <div className={style.viewOrganizerTwo}>
                                <div>
                                    <Chip title={"Phase de vote"} backgroundColor={"#000"} color={"#fff"} />
                                </div>
                                <div>
                                    <p>Fin le {new Date(entity.voting_end_date).toLocaleDateString("fr-FR", {
                                        year: "numeric",
                                        month: "long",
                                        day: "numeric",
                                    })}</p>
                        ...
                        <div className={style.viewOrganizerStats}>
                            <Chip title={entity.numberOfUser} icon={"user-plus"} backgroundColor={"#F5F5F5"} />
                            <Chip title={entity.pictures?.length} icon={"camera"} backgroundColor={"#F5F5F5"} />
                            <Chip title={entity.numberOfVotes} icon={"like"} backgroundColor={"#F5F5F5"} />
                            <Chip />
                        </div>
                    </div>
                </div>
