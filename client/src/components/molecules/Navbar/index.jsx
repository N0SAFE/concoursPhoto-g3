    ...
    function list(disposition) {
        return disposition.map((item, index) => {
            if (item.type === "classic") {
                ...
                return <Dropdown links={item.links} title={item.title} requireLogin={isLogged} requireToken={true} />;
                ...
                return <Button name={item.title} borderRadius={"5px"} padding={"5px 10px"} icon={item.icon} onClick={() => (typeof item.action === "function" ? item.action() : navigate(item.to))} />;
            }
        });
    }
